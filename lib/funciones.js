...
function CerrarRecep(){
	jQuery.get("http://corprocessu.com:8000/CerrarRecep/"+localStorage.mov+"/alvaro",function(resultado1){
...
function ProcesarItem(){
	...
	if(item!="" && cant>0 && almacen!=""){
		...
		jQuery.get("http://corprocessu.com:8000/IngresarRecepcionDet/"+sec+"/"+localStorage.mov+"/"+almacensec+"/"+cant+"/alvaro/"+monto,function(resultado1){
			redireccionar("NewRecep.html");
		});
	}
}
...
function IngresarRecep(){
	...
	jQuery.get("http://corprocessu.com:8000/IngresarRecep/"+id+"/"+desc+"/"+fecha+"/"+localStorage.FTrans+"/"+resp+"/"+nro+"/alvaro",function(resultado){
		miRes = jQuery.parseJSON(resultado);
		$.each(miRes, function(i, elemento){
...
function MenuCodif(){
	...
	jQuery.get("http://corprocessu.com:8000/IngresarCod/"+id+"/"+desc+"/"+fecha+"/"+respsec+"/alvaro",function(resultado){
		miRes = jQuery.parseJSON(resultado);
		$.each(miRes, function(i, elemento){
			localStorage.mov=elemento.sec;
		});
	...
	if(CI!="" && localStorage.almacen!=""){	
		...
		jQuery.get("http://corprocessu.com:8000/ValidarCod/"+CI+"/"+CF,function(resultado){
			...
			if(error){
				...
				jQuery.get("http://corprocessu.com:8000/GenerarCod/"+localStorage.matcat+"/"+CI+"/"+CF+"/"+almacensec+"/alvaro/"+localStorage.mov,function(resultado){
					 redireccionar('Codificacion.html');
				});
			}	
		});
